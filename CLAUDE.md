...
- Rendered pages are serialized through an XML parser, which only knows the five XML entities, so anything reaching the page as raw text must survive that — and a failure aborts the whole render, not just its own slide. Author content is handled for you: `include()` escapes in `InternalUtils.fixIndents`, and `markdownSlide`/`htmlSlide` bodies pass through `InternalUtils.toXmlSafeEntities`, which repairs a bare `&` and converts named entities XML does not declare (`&nbsp;`, `&mdash;`) without touching tags. Text kslides emits itself is not covered: **injected scripts** must contain no bare `&` or `<` (see `AMP` in `FollowAlong.kt`), and the same applies to raw markup reaching `css {}` or the corner-link SVGs.
