...
- Rendered pages are serialized through an XML parser, which only knows the five XML entities, so anything reaching the page as raw text must survive that. Content read by `include()` (and so by `slideDefinition`/`smallSlideDefinition`) is escaped for you in `InternalUtils.fixIndents`. Text you emit yourself is not: **injected scripts** must contain no bare `&` or `<` (see `AMP` in `FollowAlong.kt`), and neither must raw markup passed to `htmlSlide`, `css {}`, or the corner-link SVGs — a literal `&mdash;` or `&nbsp;` in any of those aborts the whole render, not just its own slide.
