...
- Rendered pages are serialized through an XML parser, which only knows the five XML entities, so anything reaching the page as raw text must survive that — and a failure aborts the whole render, not just its own slide. Two sinks do the repair, split by destination: `rawHtml` for element content (decodes `&nbsp;`/`&mdash;` to the characters they name) and the internal `rawSource` for `<script>`/`<style>` bodies (escapes instead, since those do not decode entities and a decoded `&&` would rewrite the JavaScript). Both also neutralize a `]]>` run. **A bare `<` is still fatal** and cannot be repaired either way, since slide content legitimately carries tags: `markdownSlide { "val x: List<String>" }` dies with `The element type "String" must be terminated`. Write `&lt;` or fence it as code.
