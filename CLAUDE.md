...
- Rendered pages are serialized through an XML parser, which only knows the five XML entities, so anything reaching the page as raw text must survive that. **Injected scripts** must contain no bare `&` or `<` (see `AMP` in `FollowAlong.kt`) — they are emitted unescaped by design, since escaping would break the JavaScript. Content read by `include()` (and so by `slideDefinition`/`smallSlideDefinition`) is escaped for you with `escapeXml10`, which emits only XML-legal entities; it used to use `escapeHtml4`, whose named entities — `&mdash;` for an em dash, say — are undefined in XML and aborted the whole render.
