...
- Rendered pages are serialized through an XML parser, which only knows the five XML entities, so text reaching it as raw markup must survive that — and a failure aborts the whole render, not just its own slide. `rawHtml` repairs what it can for element content (`InternalUtils.xmlSafeAsMarkup`: an ampersand opening no declared reference, and a `]]>` run). **A bare `<` there is still fatal** and is not repairable, since slide content legitimately carries tags — `markdownSlide { "val x: List<String>" }` dies with `The element type "String" must be terminated`, and fencing it as code does *not* help. Write `&lt;`. Script and style bodies avoid all of this: they are emitted with kotlinx.html's `+text`, a text node the parser never sees, so any character is fine there.
