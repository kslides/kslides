...
Documentation lives at the [Zensical docs site](https://kslides.github.io/kslides/),
with the [API reference](https://kslides.github.io/kslides/api-docs/) generated by Dokka.

`kslides` 1.4.0 is published to Maven Central under the `com.kslides` group ID
(`com.kslides:kslides-core`, `com.kslides:kslides-letsplot`,
`com.kslides:kslides-export`). See
[CHANGELOG.md](CHANGELOG.md) and [RELEASE_NOTES.md](RELEASE_NOTES.md) for the
list of breaking changes from `0.x` (notably `plotly { }` → `letsPlot { }` and
the JitPack → Maven Central coordinate change).
...
* A `playgroundConfig{}` block ([example](https://www.srcref.com/github?account=kslides&repo=kslides&branch=master&path=%2Fkslides-examples%2Fsrc%2Fmain%2Fkotlin%2FSlides.kt&bregex=playgroundConfig+%5C%7B&boccur=1&boffset=0&btopd=true&eregex=%2F%2F+playgroundConfig+end&eoccur=1&eoffset=-1&etopd=true))
  ...
  * The `fontSize` option sizes the code in the editor and the run-output pane together, cascading like every
  other config value. Line spacing follows on its own; a `lineHeight` option changes that ratio. Prefer absolute
  units — the Playground renders in its own iframe document, so `em` resolves against that document's root font
  size rather than the surrounding slide's.

  * The `css{}` block inside the `playgroundConfig{}` block specifies the css for the playground content that is
  displayed in the iframe — the escape hatch for Playground internals `fontSize` does not cover. It is written to
  the iframe head after the generated `fontSize` rules, so a rule declared here still wins at equal specificity.
  The css values can be specified via multi-line strings or using the CSS DSL.
...
Alongside the slide background options, `slideConfig{}` carries three font properties:
...
Like the rest of the cascade, setting these in `presentationConfig{ slideConfig{ } }` applies
them deck-wide; setting them on an individual slide overrides that. `slideDefinition()` takes a
trailing config block for the same purpose:
...
deck-wide in the presentation/global `slideConfig{}`.
...
|---------------------------|-----------------|--------------------------------------------|----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `slideBackground()`       | MarkdownSlides  | Sets a markdown slide's background color. Append it to a line of body text — on its own line it is ignored | [example](https://www.srcref.com/github?account=kslides&repo=kslides&branch=master&path=%2Fkslides-examples%2Fsrc%2Fmain%2Fkotlin%2FSlides.kt&bregex=%2F%2F%5Cs*background-markdown%5Cs%2Bbegin&boccur=1&boffset=1&btopd=true&eregex=%2F%2F%5Cs*background-markdown%5Cs%2Bend&eoccur=1&eoffset=-1&etopd=true)                 |
...
| `DslSlide.mermaid()`          | Embeds a Mermaid diagram     | [docs](https://kslides.github.io/kslides/extensions/mermaid/)                                                                                                                                                                                             |
...
When serving over HTTP, set `output.followAlong = true` to keep a remote audience on your slide
...
The presenter opens the deck with `?present=<token>` — the presenter URLs, token included, are logged
at startup — and everyone else uses the plain deck URL. Their browsers follow the presenter's slide
and fragment position live. A badge reads `● LIVE` while following; clicking it breaks away for free
navigation, and clicking again snaps back to wherever the presenter is now. Late joiners land on the
current slide, not slide one.

The audience is read-only by construction: the server ignores frames from anyone who isn't the
presenter. There is one presenter per deck — a newcomer with a valid token supersedes the previous
connection, which demotes itself to a viewer. Set `presenterToken` for a predictable URL instead of a
random token per launch. This is deliberately lightweight auth: the token travels in the URL, so it
gates control of a slide deck, not a security boundary. See the
[Presenting docs](https://kslides.github.io/kslides/presenting/).

### PDF Export

The optional `kslides-export` module prints a deck to PDF in one command, replacing the manual
`?print-pdf`-and-print-to-file ritual:
...
`exportPdf()` takes the same block as `kslides{}`, serves the presentations from an ephemeral-port
server, and prints each deck through headless Chromium ([Playwright](https://playwright.dev/java/)),
waiting for reveal.js' `pdf-ready` event and any Mermaid diagrams so pages are sized by the deck's own
print stylesheet. Configure it with the `pdf { }` block inside `output { }` — output directory, page
size, per-presentation `exclude()`, an optional first-slide `previewPng`, and `browserChannel` to drive
an installed Chrome/Edge instead of downloading Chromium. See the
[PDF export docs](https://kslides.github.io/kslides/pdf-export/).

### Custom Themes

`customTheme { }` inside `presentationConfig { }` layers typed overrides on a stock reveal.js theme, so
brand styling doesn't require reverse-engineering theme variables:
...
presentationConfig {
  customTheme {
    ...
    headingTextTransform = TextTransform.none  // disable the stock UPPERCASE headings
    codeFont = "Menlo, Consolas, monospace"
    logo("images/logo.png", position = LogoPosition.BOTTOM_RIGHT, size = 90.px)
  }
}
```

Eighteen typed properties map to reveal.js' `--r-*` theme variables, with
`customProperty("--r-…", …)` for anything the DSL doesn't model. Only assigned properties are emitted,
and the block cascades global → presentation like every other config. `logo()` pins a brand image to a
corner of every slide and every exported PDF page. See the
[Styling docs](https://kslides.github.io/kslides/styling/) and the
[theme example deck](https://kslides.github.io/kslides/docs/theme.html).
...
`mermaid()` renders a [Mermaid](https://mermaid.js.org) diagram client-side, using a Mermaid
runtime bundled with kslides. Unlike Kroki (below), it needs no network access or external
server, so decks keep rendering offline:
...
The Mermaid runtime is emitted only for presentations that actually contain a `mermaid()`
call, and Mermaid's dark theme is selected automatically for dark reveal.js themes. Because
`mermaid()` takes a `String`, the diagram source can live in a separate file or URL via
`include()`. Diagrams render as their slide becomes visible; print view renders the whole
deck up front.

Use `mermaid()` for Mermaid diagrams and Kroki for formats Mermaid doesn't cover
(PlantUML, GraphViz, D2, …). See the [Mermaid docs](https://kslides.github.io/kslides/extensions/mermaid/).
