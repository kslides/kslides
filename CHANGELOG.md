...
- Slide background assets — `slideConfig`'s `backgroundImage`, `backgroundIframe`,
  ...
  shorthand takes a color *or* an image, so only values ending in a known image
  extension (`.png`, `.jpg`, `.svg`, …) resolve — a color name, `#hex`, or
  `rgb()` is emitted untouched. Prefer `backgroundImage` when you mean an image.
- Author-supplied asset paths — `customTheme { logo(...) }`, `topLeftSvgSrc`,
  ...
  **If you hand-compensated for this with a `../` path on a nested deck, drop the
  ...
  which are navigation targets and deliberately left alone; and image paths
  written inside Markdown or HTML slide content, which kslides hands to reveal.js
  unparsed.
...
- `playground { }`, `letsPlot { }`, and `diagram { }` content now loads on decks
  ...
  addresses the routes absolutely, which is where they are registered. Same depth
  bug as the reveal.js asset-link fix below — the example decks all sit at the
  output root, which is why neither surfaced there. Decks at the output root are
  unaffected: their generated output is byte-identical. Note that asset paths you
  supply yourself — `logo()`, the corner-link SVGs, image paths in slide content —
  are still emitted verbatim, so write them relative to the deck rather than to the
  output root.
